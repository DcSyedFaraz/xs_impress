                    ...
                    <div class="filters">
                        ...
                        <form action="" method="GET">
                            ...
                            @if($filterSku || $filterXsSku || $filterCategory || $filterSupplier)
                                <a href="{{ URL::to('product/deleted') }}" class="btn btn-light btn-sm mt-3">Clear Filter</a>
                            @endif
                        </form>
                    </div>
                   <div class="clearfix mt-4">
                       <div class="float-start">
                           Total no of products: <b>{{ $productClass->getDeletedCollectionSize($filterXsSku, $filterSku, $filterSupplier, $filterCategory) }}</b>
                       </div>
                       <div  class="float-end">
                           <form action="{{ URL::to('product/recover') }}" method="POST">
                               @csrf
                               <input type="hidden" id="selected-product-ids" name="product_ids" value="">
                               <button href="{{ URL::to('product') }}" class="btn btn-success btn-sm btn-recover-products" disabled>Recover Products</button>
                           </form>
                       </div>
