@php
    ...
    $filterSupplier =
        request()->has('supplier') && request()->input('supplier') != 'Select' ? request()->input('supplier') : '';
    $filterCategory =
        request()->has('category') && request()->input('category') != 'Select' ? request()->input('category') : '';
@endphp
<x-app-layout>
    <x-slot name="header">
                    ...
                    <div class="">
                        <div class="filters">
                            ...
                            <form action="" method="GET">
                                <div class="row">
                                    <div class="col-12 col-md-3">
                                        <div class="form-group">
                                            <label>XS Sku</label>
                                            <input type="text" class="form-control" name="xs_sku"
                                                value="{{ $filterXsSku }}">
                                        </div>
                                    </div>
                                    <div class="col-12 col-md-3">
                                            ...
                                            <select class="form-control" name="supplier">
                                                ...
                                                @foreach ($suppliers as $supplier)
                                                    <option value="{{ $supplier->id }}"
                                                        @if ($filterSupplier == $supplier->id) selected @endif>
                                            ...
                                            <select class="form-control" name="category">
                                                ...
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->key }}"
                                                        @if ($filterCategory == $category->key) selected @endif>
                                                        {{ $category->de }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                @if ($filterSku || $filterXsSku || $filterCategory || $filterSupplier)
                                    <a href="{{ URL::to('product/deleted') }}" class="btn btn-light btn-sm mt-3">Clear
                                        Filter</a>
                                @endif
                            </form>
                        </div>
                        <div class="clearfix mt-4">
                            <div class="float-start">
                                ...
                                <b>{{ $productClass->getDeletedCollectionSize($filterXsSku, $filterSku, $filterSupplier, $filterCategory) }}</b>
                            </div>
                            <div class="float-end">
                                <form action="{{ URL::to('product/recover') }}" method="POST">
                                    @csrf
                                    <input type="hidden" id="selected-product-ids" name="product_ids" value="">
                                    <button href="{{ URL::to('product') }}"
                                        class="btn btn-success btn-sm btn-recover-products" disabled>Recover
                                        Products</button>
                                </form>
                            </div>
                            <div class="float-end pt-1 mx-3">
                                <input type="checkbox" class="show-hide-variant" name="1" <?php if(isset($_COOKIE['product_grid_view_mode'])): ?>
                                    checked <?php endif; ?>>
                                Hide Variants
                            </div>
                        </div>
                        @if (count($products))
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if (!$product->parent_id)
                                                    <input type="checkbox" class="child-checkbox" name="product_ids[]"
                                                        value="{{ $product->id }}">
                                                @endif
                                            </td>
                                            <td>{{ $productClass->getProductCustomSku($product->sku)->custom_sku }}
                                            </td>
                                            <td style="width: 125px">
                                                <div
                                                    style="width: 85px; height: 85px; background-image: url({{ $product->image_url }}); background-size: cover">
                                                </div>
                                            </td>
                                            <td>
                                            ...
                                            <td>{{ $product->parent_id ? 0 : $productClass->getVariantCount($product->id) }}
                                            </td>
                                            <td>
                            ...
                            <div class="d-flex mt-5">
                                <div class="mx-auto"> {{ $products->appends(request()->except('page'))->links() }}
                                </div>
                            </div>
                        @else
                            <h6>
    ...
    <script>
        $(document).ready(function() {
            $('.show-hide-variant').on('click', function() {
                if ($(this).is(':checked')) {
                    document.cookie =
                        "product_grid_view_mode=without_variant; expires=Thu, 18 Dec 2050 12:00:00 UTC; path=/";
                    window.location.reload();
                } else {
                    document.cookie =
                        "product_grid_view_mode=without_variant; expires=Thu, 18 Dec 2005 12:00:00 UTC; path=/";
                    window.location.reload();
                }
            });
        });
        ...
        $('.parent-checkbox').on('click', function() {
            if (!$(this).is(':checked')) {
                ...
                $('.btn-recover-products').prop('disabled', true);
                productIds = [];
            ...
            $.each($('.child-checkbox'), function(i, v) {
                $('.btn-recover-products').prop('disabled', false);
                $(v).prop("checked", true);
