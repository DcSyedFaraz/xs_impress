...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight clearfix">
            <div style="border-radius: 5px; display: inline-block; width: 85px; height: 85px; background-image: url({{ $productClass->getProductImage($product->id)->image_url }}); background-size: cover"></div>
            <div style="display: inline-block; margin-left: 10px">
                {{ $productClass->getProductDetailInDe($product->id)->name }}
                <br>
                <small style="font-size: 16px">{{ $productClass->getProductCustomSku($product->sku)->custom_sku }}</small>
                <br>
            ...
            <a href="{{ $_COOKIE['filter_product_page'] }}" class="btn btn-sm btn-danger float-end">Go Back to Products</a>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <p>
                        @foreach(\App\Models\ProductImage::where('product_detail_id', $productClass->getProductDetailInDe($product->id)->id)->get() as $pImage)
                            <div style="border-radius: 5px; display: inline-block; width: 150px; height: 150px; background-image: url({{ $pImage->url }}); background-size: cover"></div>
                        @endforeach
                    </p>
                </div>
            </div>
                            ...
                            @foreach($variants as $key => $variant)
                                <tr>
                                    ...
                                    <td style="width: 125px">
                                        <div style="width: 85px; height: 85px; background-image: url({{ $variant->image_url }}); background-size: cover"></div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @php
                                            $allStaticContent = $productClass->getProductStatisCategory($variant->sku);
                                        @endphp
                                        {{ $allStaticContent }}
                                    </td>
                                    <td>
            ...
            @php
                $buyingPrice = \App\Models\ProductPriceCountryBased::where([['product_id', $product->id], ['type', 'General Buying Price']])->get();
                $sellingPrice = \App\Models\ProductPriceCountryBased::where([['product_id', $product->id], ['type', 'Recommended Selling Price']])->get();
            @endphp
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-5">
                <div class="p-6 text-gray-900">
                    @if($buyingPrice)
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div>
                        ...
                        @foreach($printingOptions as $key => $printingOption)
                            <li class="mb-4">
                                @php
                                    $positionTextDe = $productClass->printingPositionsTextDe($printingOption->id);
                                    $positionOptions = $productClass->printingPositionOptions($printingOption->id);
                                ...
                                <div>
                                    ...
                                    <div>
                                        ...
                                        @foreach($positionOptions as $positionOption)
                                            @php
                                                $optionSellingPrice = $productClass->printingSellingPrice($positionOption->id);
                                                $options = json_decode($positionOption->imprint_texts, true);
                                            @endphp
                                            @if(isset($options['de']))
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ URL::to('product/update/'.$product->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control js-example-basic-single" name="category[]" style="height: 300px; margin-top: 10px" multiple>
                                @foreach($categories as $category)
                                    <option value="{{ $category->key }}" @if(in_array($category->de, explode(' | ', (string)$productClass->getProductStatisCategory($product->sku)))) selected @endif>{{ $category->de }}</option>
                                @endforeach
                            </select>
                        </div>
